import CauldronV2Flat from './CauldronV2Flat.json';
import CauldronV2 from './CauldronV2.json';
import CauldronV2CheckpointV1 from './CauldronV2CheckpointV1.json';
import CauldronV2Multichain from './CauldronV2Multichain.json';
import CauldronV2FTM from './CauldronV2FTM.json';
import CauldronV3 from './CauldronV3.json';
import WhitelistedCauldronV3 from './WhitelistedCauldronV3.json';
import CauldronV3_2 from './CauldronV3_2.json';
import CauldronV4 from './CauldronV4.json';
import PrivilegedCauldronV4 from './PrivilegedCauldronV4.json';

export {
  CauldronV2Flat,
  CauldronV2CheckpointV1,
  CauldronV2,
  CauldronV2Multichain,
  CauldronV2FTM,
  CauldronV3,
  WhitelistedCauldronV3,
  CauldronV3_2,
  CauldronV4,
  PrivilegedCauldronV4,
};
